...
export class QuendooClaudeIntegration {
  ...
  async processMessage(message, conversationId, model = 'claude-3-5-haiku-20241022', systemPrompt = null, quendooApiKey = null, hotelId = null, onToolProgress = null, attachedDocumentIds = []) {
    ...
    try {
      ...
      // Keep history manageable (last 12 messages = ~3 turns)
      // Analysis shows: 12 messages with 500-char excerpts = ~5,500 tokens (2.7% of limit) - SAFE
      // This provides better context for Claude while staying well under token limit
      ...
      let userMessage = 'Възникна грешка при обработка на заявката.';

      if (error.status === 400) {
        userMessage = 'Невалидна заявка. Опитай с различна формулировка.';
      } else if (error.status === 429) {
        userMessage = 'Твърде много заявки. Моля изчакай малко и опитай отново.';
      } else if (error.status === 500 || error.status === 503) {
        userMessage = 'Проблем със сървъра на Claude. Моля опитай отново след малко.';
      }
    ...
    try {
      ...
      while (loopCount < maxLoops) {
        ...
        try {
          ...
          // Check if error is "unexpected tool_use_id" (orphan tool_result)
          if (apiError.status === 400 && apiError.error?.message?.includes('unexpected `tool_use_id`')) {
            console.warn('[Quendoo Streaming] Detected orphan tool_result, cleaning history and retrying...');

            // Remove last message if it contains tool_results
            if (history.length > 0 && history[history.length - 1].role === 'user') {
              const lastMessage = history[history.length - 1];
              if (Array.isArray(lastMessage.content) && lastMessage.content.some(c => c.type === 'tool_result')) {
                console.log('[Quendoo Streaming] Removing orphan tool_result message from history');
                history.pop();
                requestParams.messages = history;

                // Retry request with cleaned history
                try {
                  response = await this.anthropic.messages.create(requestParams);
                  console.log('[Quendoo Streaming] Retry successful after history cleanup');
                } catch (retryError) {
                  console.error('[Quendoo Streaming] Retry failed after cleanup:', retryError);
                  throw new Error('Грешка в историята на разговора. Моля започни нов разговор.');
                }
              } else {
                // Not a tool_result issue, rethrow
                throw apiError;
              }
            } else {
              // Can't clean history, rethrow
              throw apiError;
            }
          } else {
        ...
        // Log actual token usage from Claude API
        if (response.usage) {
          const inputTokens = response.usage.input_tokens || 0;
          const outputTokens = response.usage.output_tokens || 0;
          const totalTokens = inputTokens + outputTokens;
          const percentUsed = ((inputTokens / 200000) * 100).toFixed(1);

          console.log(`[Quendoo Streaming] Token usage: input=${inputTokens.toLocaleString()}, output=${outputTokens.toLocaleString()}, total=${totalTokens.toLocaleString()} (${percentUsed}% of 200K limit)`);

          // Alert if approaching limit (>90%)
          if (inputTokens > 180000) {
            console.error(`⚠️ [Quendoo Streaming] TOKEN LIMIT WARNING: ${inputTokens.toLocaleString()} / 200,000 tokens (${percentUsed}%) - Consider clearing conversation history!`);
          }
        }
      ...
      if (error.message && error.message.includes('започни нов разговор')) {
        // Already a user-friendly message from orphan cleanup
        throw error;
      } else if (error.status === 400) {
        userMessage = 'Невалидна заявка. Опитай с различна формулировка или започни нов разговор.';
      } else if (error.status === 429) {
        userMessage = 'Твърде много заявки. Моля изчакай малко и опитай отново.';
      } else if (error.status === 500 || error.status === 503) {
        userMessage = 'Проблем със сървъра на Claude. Моля опитай отново след малко.';
      } else if (error.code === 'ECONNREFUSED' || error.code === 'ETIMEDOUT') {
        userMessage = 'Не мога да се свържа със сървъра. Провери интернет връзката.';
      } else if (error.message && error.message.includes('tool')) {
        userMessage = 'Инструментът не успя да се изпълни. Опитай с друга формулировка.';
      }
