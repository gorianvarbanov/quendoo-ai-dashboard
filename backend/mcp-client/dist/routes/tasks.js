 ...
 * GET /api/tasks?hotelId=hotel_123
 */
...
router.post('/', async (req, res) => {
  try {
    const { hotelId, name, description, schedule, action, enabled = true } = req.body;

    // Validation
    if (!hotelId || !name || !schedule || !action) {
      return res.status(400).json({
        error: 'Missing required fields: hotelId, name, schedule, action'
      });
    }

    if (!action.type || !['mcp_tool', 'api_call'].includes(action.type)) {
      return res.status(400).json({
